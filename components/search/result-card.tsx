    ...
    >
      <div className="grid size-16 shrink-0 place-items-center overflow-hidden rounded-md bg-ink-sunken">
        {card.medium === "image" && image?.url ? (
          ...
          <span className="font-serif text-lg text-bone-46">“”</span>
        )}
      </div>
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="meta mt-0.5 truncate">
          @{card.artist.handle} · <span className="meta-caps">{card.medium}</span> · {formatPieceDate(card.published_at)}
        </p>
        {card.medium === "words" && card.body && (
          <p className="mt-1 line-clamp-1 font-serif text-sm text-bone-46">{card.body}</p>
        )}
      </div>
